...
export default function WhatWeDo({ whatWeDoGridTexts }) {
  return (
    <div className="lg:ms-28 mt-20 lg:pe-10">
      <div className="flex lg:flex-row flex-col lg:gap-10 gap-20 lg:items-center">
        ...
        <div className="lg:w-[60%] w-full flex flex-col gap-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:gap-20 gap-14">
            {whatWeDoGridTexts.map((item) => {
              return (
                <div key={item.id}>
                  <Image
                    src={item.iconPath}
                  ...
                  <h3 className={` ${item.id == 0 && "mt-6"} text-xl mt-4 text-slate-600 font-semibold`}>
                    {item.title}
                  </h3>
